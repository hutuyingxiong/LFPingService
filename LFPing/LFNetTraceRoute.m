...
@implementation LFNetTraceRoute
...
- (Boolean)doTraceRoute:(NSString *)host
{
    ...
    NSArray *serverDNSs = [LFNetGetAddress getDNSsWithDormain:host];
    ...
    while (ttl <= maxTTL) {
        ...
        for (int try = 0; try < maxAttempts; try ++) {
            startTime = [LFNetTimer getMicroSeconds];
            //发送成功返回值等于发送消息的长度
            ...
            if (FD_ISSET(recv_sock, &readfds) > 0) {
                ...
                if ((res = recvfrom(recv_sock, buf, 100, 0, (struct sockaddr *)&fromAddr, &n)) <
                    ...
                    icmp = true;
                    delta = [LFNetTimer computeDurationSince:startTime];
